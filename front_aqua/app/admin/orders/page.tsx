...
export default function OrdersDashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    orders.forEach((o) => {
      const pStatus = getOrderPresentationStatus(o);
      const sum = o.totalAmount ?? o.TotalAmount ?? 0;

      if (pStatus.type !== "cancelled") {
        totalRevenue += sum;
        activeOrdersCount++;
      }

      const tickets = o.tickets || o.Tickets || [];
      tickets.forEach((t) => {
        if ((t.status || t.Status) === "Used") validatedTicketsCount++;
      });
    });

    return { totalRevenue, validatedTicketsCount, activeOrdersCount };
  }, [orders]);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl">
            <span className="text-xs text-slate-400 font-medium">Активних замовлень</span>
            <div className="text-2xl font-black text-sky-400 mt-1">
              {stats.activeOrdersCount}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl">
            <span className="text-xs text-slate-400 font-medium">Валідованих квитків</span>
            <div className="text-2xl font-black text-purple-400 mt-1">
              {stats.validatedTicketsCount}
            </div>
          </div>
          <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl">
            <span className="text-xs text-slate-400 font-medium">Каса за період</span>
